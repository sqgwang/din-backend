...
// 允许的前端来源（只写 host，不带路径）。GitHub Pages: https://<username>.github.io
const ALLOWED_ORIGINS = [
  'https://sqgwang.github.io',
  // 如需本地联调，可临时加入：'http://localhost:5500', 'http://127.0.0.1:5500'
];

// 信任代理（让 secure cookie 在反向代理/Render 下生效）
app.set('trust proxy', 1);

// ----------- CORS -----------
app.use(cors({
  origin(origin, cb) {
    if (!origin) return cb(null, true);         // 允许 Postman/curl 等无 Origin 的请求
